...
export LOGFILE_PATH="/var/log/nginx"
export FILE_PREFIX="error"
export LOG_TYPE="nginx"
...
grab_all_ips() {
  cat "$LOGFILE_PATH"/"$FILE_PREFIX".log "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9] | awk -F 'client: ' '{print $2}'| awk -F ', server' {'print $1'} > "$LOG_TYPE"_"$FILE_PREFIX".ips
  zcat "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9].gz | awk -F 'client: ' '{print $2}'| awk -F ', server' {'print $1'} >> "$LOG_TYPE"_"$FILE_PREFIX".ips
...
grab_activity_time(){
  ...
  do
    > $ip.unsorted
    cat "$LOGFILE_PATH"/"$FILE_PREFIX".log "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9] | grep $ip | awk {'print $1,$2'} >> $ip.unsorted 
    zcat "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9].gz | grep $ip | awk {'print $1,$2'} >> $ip.unsorted
