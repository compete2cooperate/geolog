...
grab_activity_time(){
  ...
  do
    > $ip.unsorted
    cat "$LOGFILE_PATH"/"$FILE_PREFIX".log "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9] | grep $ip | awk -F "[" -e '{print $2}' | awk -F "]" '{print $1}'| sed 's/\//-/g'| sed 's/:/\ /' | sed 's/\+0000//g' >> $ip.unsorted
    zcat "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9].gz | grep $ip | awk -F "[" -e '{print $2}' | awk -F "]" '{print $1}'| sed 's/\//-/g'| sed 's/:/\ /' | sed 's/\+0000//g' >> $ip.unsorted
