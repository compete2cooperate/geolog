...
grab_all_ips() {
  ...
  cat "$LOGFILE_PATH"/"$FILE_PREFIX" "$LOGFILE_PATH"/"$FILE_PREFIX".[0-9] | awk -F 'SRC=' {'print $2'} | awk -F 'DST=' {'print $1'} | sort -u > "$LOG_TYPE"_"$FILE_PREFIX".ips
  #zcat "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9].gz | awk -e '{print $1}' >> "$LOG_TYPE"_"$FILE_PREFIX".ips
  zcat "$LOGFILE_PATH"/"$FILE_PREFIX".[0-9].gz | awk -F 'SRC=' {'print $2'} | awk -F 'DST=' {'print $1'} | sort -u >> "$LOG_TYPE"_"$FILE_PREFIX".ips
...
grab_activity_time(){
  ...
  do
    > $ip.unsorted
    cat "$LOGFILE_PATH"/"$FILE_PREFIX" "$LOGFILE_PATH"/"$FILE_PREFIX".[0-9] | grep $ip | awk {'print $1, $2, $3'} >> $ip.unsorted
    zcat "$LOGFILE_PATH"/"$FILE_PREFIX".[0-9].gz | grep $ip |  awk {'print $1, $2, $3'} >> $ip.unsorted
