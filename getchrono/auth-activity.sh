...
grab_all_ips() {
  ...
  cat "$LOGFILE_PATH"/"$FILE_PREFIX".log "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9] | awk -F 'from' {'print $2'} | awk -F 'port' {'print $1'} | sort -u > "$LOG_TYPE"_"$FILE_PREFIX".ips
  #zcat "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9].gz | awk -e '{print $1}' >> "$LOG_TYPE"_"$FILE_PREFIX".ips
  zcat "$LOGFILE_PATH"/"$FILE_PREFIX".log.[0-9].gz | awk -F 'from' {'print $2'} | awk -F 'port' {'print $1'} | sort -u >> "$LOG_TYPE"_"$FILE_PREFIX".ips
